...
export default function DOMSidebar({ orderBook, activePair, theme = "dark", language = "EN" }: DOMSidebarProps) {
  ...
  const [isFearGreedCollapsed, setIsFearGreedCollapsed] = useState<boolean>(() => {
    return storage.get("procluster_fear_greed_collapsed") === "true";
  });

  const toggleFearGreed = () => {
    setIsFearGreedCollapsed(prev => {
      const next = !prev;
      storage.set("procluster_fear_greed_collapsed", next ? "true" : "false");
      return next;
    });
  };
  ...
  return (
    <div className={`rounded-2xl p-4 flex flex-col h-full shadow-2xl relative overflow-hidden text-xs transition-all duration-300 liquid-glass-card ${
      ...
      <div className={`rounded-xl p-2 border transition-all duration-300 ${
        isFearGreedCollapsed ? "mb-0" : "mb-2"
      } ${
        isLight 
        ...
        <div 
          ...
          className={`flex items-center justify-between gap-1.5 cursor-pointer hover:opacity-80 transition-opacity select-none ${
            isFearGreedCollapsed ? "mb-0" : "mb-1.5"
          }`}
        >
          ...
          <div className={`p-0.5 rounded transition-colors duration-150 ${isLight ? "text-slate-400 hover:text-slate-600 hover:bg-slate-200/50" : "text-slate-500 hover:text-slate-300 hover:bg-white/5"}`}>
            {isFearGreedCollapsed ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronUp className="w-3.5 h-3.5" />}
          </div>
        </div>

        {!isFearGreedCollapsed && (
          <>
